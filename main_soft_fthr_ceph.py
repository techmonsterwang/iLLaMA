...
def get_args_parser():
    ...
    # soft mask settings
    parser.add_argument('--soft_mask', type=float, default=1.0, metavar='PCT',
                        help='Soft mask rate (default: 1.0)')
    parser.add_argument('--mask_mode', type=str, default='standard', choices=['standard', 'soft'], help='mask mode')
    parser.add_argument('--mask_schedule', type=str, default='constant', choices=['constant', 'linear'], 
                        help='mask schedule for soft mask')
    parser.add_argument('--cutoff_soft', type=int, default=0, 
                        help='if mask_mode is early / late, this is the epoch where soft mask ends / starts')
...
def main(args):
    ...
    if args.finetune:
        ...
        # I add these code, interpolate position embedding
        print('here')
        ...
        new_pos_embed = torch.cat((pos_tokens, extra_tokens), dim=1)
        checkpoint_model['pos_embed'] = new_pos_embed
        # interpolate position embedding done
    ...
    # Mask schedule enabled.
    schedules['sm'] = mask_scheduler(
        args.soft_mask, args.epochs, num_training_steps_per_epoch,
        args.cutoff_soft, args.mask_mode, args.mask_schedule)
    print("Min MASK = %.7f, Max MASK = %.7f" % (min(schedules['sm']), max(schedules['sm'])))  
