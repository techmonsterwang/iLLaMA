...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        token_state, attn_map = self.attn(self.norm1(x), freqs_cis, mask)
        x = x + self.drop_path(token_state)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x, attn_map = blk(x, freqs_cis, mask)
            attn_map_list.append(attn_map)

        x = self.norm(x)
        return x[:, -1], attn_map_list

    def forward(self, x):
        x, attn_map_list = self.forward_features(x)
        x = self.head(x)
...
@register_model
def doit22_femto(pretrained=False, **kwargs):
...
@register_model
def doit22_nano(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def doit22_tiny(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def doit22_small(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def doit22_base(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def doit22_large(pretrained=False, **kwargs):
    model = VisionTransformer(
